# TrainLog — настройка проекта

Этот документ предназначен для публичного репозитория: **не содержит секретов** и не требует коммита `GoogleService-Info.plist`.
...
2. В поле **Apple bundle ID** введи bundle id из Xcode (например, `com.example.trainlog`).
...
⚠️ **Важно:** пример ниже подходит для разработки и демо. Перед продакшеном правила нужно ужесточать под вашу модель доступа.
